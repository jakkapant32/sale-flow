...
-- คัดลอกและรันใน pgAdmin Query Tool
...
WITH customer_ids AS (
    SELECT id, customer_code FROM customers WHERE customer_code IN ('CUS000001', 'CUS000002', 'CUS000003', 'CUS000004', 'CUS000005', 'CUS000006', 'CUS000007', 'CUS000008')
)
...
INNER JOIN customer_ids c ON c.customer_code = deal_data.customer_code;
...
INNER JOIN customer_ids c ON c.customer_code = activity_data.customer_code;

-- 5. เพิ่มคำสั่งซื้อตัวอย่าง
WITH customer_ids AS (
    SELECT id, customer_code FROM customers WHERE customer_code IN ('CUS000001', 'CUS000002', 'CUS000003', 'CUS000004', 'CUS000005')
)
...
FROM (
    VALUES 
        ...
        ('CUS000003', 45, 3, 80000.00, 5600.00, 0.00, 85600.00, 'paid'),
        ('CUS000004', 60, 4, 120000.00, 8400.00, 0.00, 128400.00, 'paid'),
        ('CUS000005', 90, 5, 200000.00, 14000.00, 0.00, 214000.00, 'unpaid')
) AS order_data(customer_code, days, seq, subtotal, tax, discount, total_amount, payment_status)
INNER JOIN customer_ids c ON c.customer_code = order_data.customer_code
ON CONFLICT (order_number) DO NOTHING;

-- 6. เพิ่มรายการสินค้าในคำสั่งซื้อ
WITH order_product_map AS (
    SELECT 
        o.id as order_id,
        o.order_number,
        c.customer_code,
...
FROM order_product_map opm
INNER JOIN products p ON p.id = opm.product_id
WHERE NOT EXISTS (
    SELECT 1 FROM order_items oi WHERE oi.order_id = opm.order_id
);
